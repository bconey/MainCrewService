...
function addCrewMember(e) {
    var addURLID = document.getElementById('addURLID').innerHTML;
    
    //alert("HERE in addCrewMember"+addURLID);
    ...
    request.open("POST", ""+addURLID+"/api/crew/", true);
...
function toggleUpdateForm(entry) { 
    //alert("HERE in toggleUpdateForm");
...
function updateCrewMember(e) {
    var updateURLID = document.getElementById('updateURLID').innerHTML;

    //alert("HERE in updateCrewMember "+updateURLID);
    ...
    request.open("PUT", ""+updateURLID+"/api/crew/" + id, true);
    request.setRequestHeader("Content-type", "application/json");
    request.send(JSON.stringify(crewMember));
...
function addToDisplay(entry) {
    //alert("HERE in addToDisplay");

    var userHtml = "<div>Name: " + entry.Name + "</div>" +
        "<div>ID: " + entry.CrewID + "</div>" +
        "<div>Rank: " + entry.Rank + "</div>" +
        "<button class=\"deleteButton\" onclick=\"remove(event,'" + entry._id.$oid + "')\" mydeletehost=avl-node3-1.fyre.ibm.com mydeleteport=9082>Delete</button>";
...
function remove(e, id) {
    var deleteURLID = document.getElementById('deleteURLID').innerHTML;

    //alert("HERE in remove "+deleteURLID);
    ...
    request.open("DELETE", ""+deleteURLID+"/api/crew/" + id, true);
    request.send();
